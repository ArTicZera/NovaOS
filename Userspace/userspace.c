...
void DesktopIcons()
{
    ...
    {
        if (dir->subdirs[i] != NULL && dir->subdirs[i]->name[0] != '\0')
        {
            int x = 10;
            int y = 10 + (index * 76);

            DrawDesktopIcon(ICON_DIR, dir->subdirs[i]->name, x, y);
            SetDesktopIndex(dir->subdirs[i]->name, 0x00, x, y);
    ...
    for (int i = 0; i < MAXFILES; i++)
    {
        ...
        {
            char* filename = dir->files[i].filename;
            ...
            //Print(filename, 0xFF00FF00);

            EXTICON icon = ICON_DEFAULT;

            if (strcmp(filename, "DOOM") == 0)
            {
                icon = ICON_DOOM;
            }
            else if (strcmp(filename, "Terminal") == 0)
            {
                icon = ICON_TERMINAL;
            }
...
void UserSpace()
{
    ...
    while (TRUE)
    {
        ...
        HandleDesktopClick(x, y, pressed);
