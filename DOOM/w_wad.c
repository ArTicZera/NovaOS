...
static void ExtendLumpInfo(int newnumlumps)
{
    ...
    newlumpinfo = CAllocateMemory(newnumlumps, sizeof(lumpinfo_t));
...
wad_file_t *W_AddFile (char *filename)
{
    ...
    static wad_file_class_t mem_class = { .Read = Mem_Read };

    static wad_file_t* embeddedwad = NULL;

    if (!embeddedwad)
    {
        embeddedwad = malloc(sizeof(wad_file_t));
    }

    embeddedwad->file_class = &mem_class;
    embeddedwad->mapped = doom1_wad;
    embeddedwad->length = doom1_wad_len;

    wad_file->file_class = &mem_class;
    wad_file->mapped = doom1_wad;
    wad_file->length = doom1_wad_len;

    wad_file = embeddedwad;
